...
const useTicketStore = create((set, get) => ({
  ...
  fetchUserTickets: async () => {
    ...
    try {
      ...
      const upcoming = tickets.filter(ticket => new Date(ticket.eventDate) >= today);
      const past = tickets.filter(ticket => new Date(ticket.eventDate) < today);
  ...
  // Buy ticket
  buyTicket: async (eventId, ticketType) => {
    set({ isLoading: true });
    try {
  ...
  verifyPayment: async (reference) => {
    set({ isLoading: true });
    try {
      const response = await axios.get(`/tickets/verify?reference=${reference}`);
      set({ isLoading: false });
